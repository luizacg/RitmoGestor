    ...
    def get(self, request):
        try:
            ...
            if(us):          # checking for authentication using token authentication.
                ...
                # getting aluno from aluno model by filtering based on user that we got.
                detalhes = Aluno.objects.get(user=user)
                serializer = api_ser.DetalhesSerializer(
                    detalhes, context={'request': request})       # Serializing the data into Json format.
    ...
    def get(self, request):
        try:
            ...
            if(token):  # checking for authentication using token authentication.
                ...
                # getting Aluno from Aluno model by filtering based on user that we got.
                aln = Aluno.objects.get(user=user)
                # using ass_list and att_list we get the classes assigned to that user
                ass_list = Assign.objects.filter(class_id_id=aln.class_id)
                # and respectively their attendance
                att_list = []
                for ass in ass_list:
                    try:
                        a = FrequenciaTotal.objects.get(
                            aluno=aln, course=ass.course)
                    except FrequenciaTotal.DoesNotExist:
                        a = FrequenciaTotal(aluno=aln, course=ass.course)
                        a.save()
                    att_list.append(a)
    ...
    def get(self, request):
        try:
            ...
            if(token):  # checking for authentication using token authentication.
                ...
                sc_list = []
                for ass in ass_list:
                    sc = AlunoCourse.objects.get(
                        aluno=aln, course=ass.course)
                    sc_list.append(sc)
                sc_total = {}
                for sc in sc_list:
                    for m in sc.marks_set.all():
                        sc_total[m.alunocourse.course.name] = m.marks1
